...
function generateMockData() {
    ...
    const statusCounts = { all: 67, pending: 15, processing: 38, completed: 14 };
    const data = [];
    ...
    for (let i = startIndex; i < endIndex; i++) {
        ...
        data.push({
            id: `MY${String(i + 1).padStart(6, '0')}`,
            batchNo: `BATCH-MY-${String(i + 1).padStart(4, '0')}`,
            declarationNo: `MY${new Date().getFullYear()}${String(i + 1).padStart(6, '0')}`,
            exportDeclarationNo: `EXP${new Date().getFullYear()}${String(i + 301).padStart(6, '0')}`,
            status: status,
            tradeTerms: ['FOB', 'CIF', 'CFR', 'DDP'][i % 4],
            foreignShipper: ['马来西亚ABC贸易公司', '吉隆坡XYZ物流', '马来西亚DEF制造', '新山GHI进出口'][i % 4],
            domesticConsignee: ['深圳市进口贸易公司', '广州国际物流', '东莞制造企业', '佛山贸易集团'][i % 4],
            transportMode: ['海运', '空运', '公路', '铁路'][i % 4],
            entryPort: ['深圳港', '广州港', '东莞港', '珠海港'][i % 4],
            departurePort: ['吉隆坡港', '槟城港', '新山港', '关丹港'][i % 4],
...
// 导出函数供其他模块使用
window.malaysiaImportDeclaration = {
    loadDeclarationData,
    viewDeclaration,
    searchDeclarations,
    resetSearch,

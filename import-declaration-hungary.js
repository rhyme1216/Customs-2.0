...
function generateMockData() {
    ...
    const statusCounts = { all: 43, pending: 8, processing: 25, completed: 10 };
    const data = [];
    ...
    for (let i = startIndex; i < endIndex; i++) {
        ...
        data.push({
            id: `HU${String(i + 1).padStart(6, '0')}`,
            batchNo: `BATCH-HU-${String(i + 1).padStart(4, '0')}`,
            declarationNo: `HU${new Date().getFullYear()}${String(i + 1).padStart(6, '0')}`,
            exportDeclarationNo: `EXP${new Date().getFullYear()}${String(i + 201).padStart(6, '0')}`,
            status: status,
            tradeTerms: ['FOB', 'CIF', 'CFR', 'DDP'][i % 4],
            foreignShipper: ['匈牙利ABC贸易公司', '布达佩斯XYZ物流', '匈牙利DEF制造', '塞格德GHI进出口'][i % 4],
            domesticConsignee: ['深圳市进口贸易公司', '广州国际物流', '东莞制造企业', '佛山贸易集团'][i % 4],
            transportMode: ['海运', '空运', '公路', '铁路'][i % 4],
            entryPort: ['深圳港', '广州港', '东莞港', '珠海港'][i % 4],
            departurePort: ['布达佩斯港', '德布勒森港', '塞格德港', '佩奇港'][i % 4],
            declareDate: getRandomDate(),
            lastUpdater: ['张三', '李四', '王五', '赵六'][i % 4],
...
// 导出函数供其他模块使用
window.hungaryImportDeclaration = {
    loadDeclarationData,
    viewDeclaration,
    searchDeclarations,
    resetSearch,

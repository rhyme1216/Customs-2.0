...
function generateMockData() {
    ...
    const statusCounts = { all: 73, pending: 18, processing: 32, completed: 23 };
    const data = [];
    ...
    for (let i = startIndex; i < endIndex; i++) {
        ...
        data.push({
            id: `VN${String(i + 1).padStart(6, '0')}`,
            batchNo: `BATCH-VN-${String(i + 1).padStart(4, '0')}`,
            declarationNo: `VN${new Date().getFullYear()}${String(i + 1).padStart(6, '0')}`,
            exportDeclarationNo: `EXP${new Date().getFullYear()}${String(i + 601).padStart(6, '0')}`,
            status: status,
            tradeTerms: ['FOB', 'CIF', 'CFR', 'FCA', 'DDU'][i % 5],
            foreignShipper: ['越南ABC制造公司', '胡志明XYZ贸易', '河内DEF工业', '海防GHI出口商'][i % 4],
            domesticConsignee: ['厦门进口贸易公司', '福州国际物流', '泉州制造企业', '莆田贸易集团'][i % 4],
            transportMode: ['海运', '空运', '公路', '铁路'][i % 4],
            entryPort: ['厦门港', '福州港', '泉州港', '漳州港'][i % 4],
            departurePort: ['胡志明港', '海防港', '岘港', '归仁港'][i % 4],
...
// 导出函数供其他模块使用
window.vietnamImportDeclaration = {
    loadDeclarationData,
    viewDeclaration,
    searchDeclarations,
    resetSearch,

...
function generateMockData() {
    ...
    for (let i = startIndex; i < endIndex; i++) {
        ...
        data.push({
            id: `TH${String(i + 1).padStart(6, '0')}`,
            batchNo: `BATCH-TH-${String(i + 1).padStart(4, '0')}`,
            declarationNo: `TH${new Date().getFullYear()}${String(i + 1).padStart(6, '0')}`,
            exportDeclarationNo: `EXP${new Date().getFullYear()}${String(i + 101).padStart(6, '0')}`,
            status: status,
            tradeTerms: ['FOB', 'CIF', 'CFR', 'DDP'][i % 4],
            foreignShipper: ['泰国ABC贸易公司', '曼谷XYZ物流', '泰国DEF制造', '清迈GHI进出口'][i % 4],
            domesticConsignee: ['深圳市进口贸易公司', '广州国际物流', '东莞制造企业', '佛山贸易集团'][i % 4],
            transportMode: ['海运', '空运', '公路', '铁路'][i % 4],
            entryPort: ['深圳港', '广州港', '东莞港', '珠海港'][i % 4],
            departurePort: ['曼谷港', '廊开港', '春蓬港', '宋卡港'][i % 4],
...
// 导出函数供其他模块使用
window.thailandImportDeclaration = {
    loadDeclarationData,
    viewDeclaration,
    searchDeclarations,
    resetSearch,
